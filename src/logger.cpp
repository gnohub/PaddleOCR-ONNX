...
Logger::Logger(Level level) {
    m_level = level;
    m_severity = getSeverity(level);
}

Logger::Severity Logger::getSeverity(Level level) {
    switch (level) {
...
Level Logger::getLevel(Severity severity) {
    string str;
    switch (severity) {
...
void Logger::log(Severity severity, const char* msg) noexcept {
    if (severity <= getSeverity(Level::WARN)
        || m_level >= Level::DEBUG)
        __log_info(getLevel(severity), "%s", msg);
}
...
shared_ptr<Logger> createLogger(Level level) {
